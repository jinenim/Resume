...
- **900개 이상** 스크립트 규모 코드베이스를 아키텍처 관점에서 정리 및 안정화
...
| **코드 아키텍처** | Manager–Utils–Info 3계층 구조 기반 대규모 코드베이스 관리 |
...
- **Unreal Engine** 기반 MORPG 클라이언트 개발
- 장비·소비·강화 아이템 등 아이템 시스템 전반 구현
- 아이템 상태 변경에 따른 클라이언트–서버 동기화 로직 설계
- **C++ 기반 메모리 관리** 및 안정성 고려한 구현 경험 축적
...
- 일본 서비스 타이틀의 **한국 서비스 로컬라이징** 작업
- 언어/리소스 분기 처리 및 UI 깨짐 이슈 해결
- 운영 중 지역화 관련 이슈 대응

#### 절벽대전 for Kakao

- **Cocos2D** 기반 실시간 액션 전투 로직 구현
- 카카오 플랫폼 연동 및 모바일 성능 최적화

#### 밀리언아서 for Kakao

- 기존 서비스 타이틀의 **카카오 플랫폼 연동** 클라이언트 작업
- 로그인·친구·초대 API 연동
- 라이브 서비스 환경의 플랫폼 종속 이슈 대응
...
- 마을 지형 시스템, UI, 퀘스트 로직 구현
- SNG + RPG 하이브리드 구조 경험 축적
...
- MMORPG 클라이언트 업데이트 및 유지보수 개발
- 신규 콘텐츠 반영 및 UI/로직 버그 수정
- 대규모 라이브 서비스 환경에서의 안정적 코드 수정 경험 축적
